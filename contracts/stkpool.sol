...
contract StakingPool is Ownable, ReentrancyGuard {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && totalStaked != 0) {
            ...
            uint256 tokenReward = multiplier * rewardPerTime;
    ...
    function updatePool() public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
        uint256 tokenReward = multiplier * rewardPerTime;
        pool.accRewardTokenPerShare += tokenReward * 1e30 / totalStaked;
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        updatePool();
        if (user.amount > 0) {
            uint256 pending = (user.amount * pool.accRewardTokenPerShare / 1e30) - user.rewardDebt;
            if (pending > 0) {
        ...
        if (_amount > 0) {
            ...
            finalDepositAmount = totalStakeTokenBalance() - preStakeBalance;
            user.amount += finalDepositAmount;
            totalStaked += finalDepositAmount;
        }
        user.rewardDebt = (user.amount * pool.accRewardTokenPerShare) / 1e30;
